...
处理任何任务时，**不要一上来全文扫描所有 docs**。按需逐步读取：

1. 先读 `docs/项目文档索引.md`，确认文档入口。
2. 再读 `docs/tasks/README.md`，确认有哪些任务线。
3. 如果用户指定任务线，只读该任务线的 `state.md`。
4. 如果用户没有指定任务线，先根据请求和索引判断最相关任务线，再读对应 `state.md`；不确定时再问。
5. 只有需要需求/设计/验收细节时，才读该任务线的 `plan.md`。
6. 只有需要核对代码实际行为、继续实现或对账时，才读该任务线的 `flow.md`。
7. 不要读取无关任务线目录；不要因为“处理问题前读 docs”而全量读取 `docs/`。
