...
| 11 | 整体链路 debug·B _click_send_verify 扩展 | ✅ DONE | wechat_rpa.py + 测试 | 120→round(120*dpi_scale)+rect校验；6 新测；94 passed |
...
## 2026-07-01 追加：整体链路 debug·B _click_send_verify 底部扩展多 DPI

- 触发：整体 debug 加微链路(派 agent 扫描+自己核验)。agent 报告多次假报,逐条核验后真实漏洞只剩 2 个(A 留讨论 + B 本轮修)。
- agent 假报复核：① SEND_SUCCESS 无关键词(假,已定义 L130) ② _fill_verify_message 75px 未缩放(假,已 int(75*scale) L1174) ③ 发送后读屏范围错(误判,Toast 在主窗口中央是有意设计) ④ 优先级重排未守卫(假,L192 已有守卫)。教训：subagent 报告须逐条核验代码。
- 漏洞B：[wechat_rpa.py:1195](python/backend/app/services/wechat_rpa.py#L1195) `bottom + 120` 是 1.0 DPI 绝对像素,高 DPI 下不够覆盖验证窗底部"确定"按钮;且 rect 未校验,窗口失效时解包抛 TypeError。
- DPI vs 分辨率：底部扩展量覆盖的是微信 UI 元素(阴影/边框/按钮)尺寸,由 DPI 决定,与分辨率无关。按 DPI 缩放,与菜单 +86 / _confirm 34 同思路。
- 修复：`extend_px = max(60, round(120*dpi_scale))`;rect 有效性校验+回退到 target_window.rect,仍无效抛 VISION_SCREENSHOT_FAILED。
- 单测：新增 `TestClickSendVerifyExtend` 6 用例(1.0/1.25/1.5 扩展=120/150/180、DPI 异常低 ≥60、left/top/right 不变、rect 无效回退)。关联测试集 94 passed(原 88+6)零回归。
- 遗留(高,留讨论)：A — [L1519](python/backend/app/services/wechat_rpa.py#L1519) 发送后"验证窗消失推断成功"(SEND_LIKELY_OK),发送失败+窗口异常关闭也判成功,误报已发送。需设计区分"窗口正常关闭=成功" vs "异常关闭=失败"。
